  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {c.outcome && (
              <Badge className={OUTCOME_COLORS[c.outcome] ?? ''}>
                {c.outcome}
              </Badge>
            )}
            {c.status && (
              ...
              <ul className="space-y-2">
                {tier1Sources.map((s: any) => (
                  <li key={s.id} className="flex items-start gap-2">
                    ...
                    <div>
                      <a href={s.url} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline line-clamp-1">
                        {s.title ?? s.url}
                      </a>
      ...
      <p className="text-xs text-gray-400 text-center pb-4">
        Data frozen as of {c.freeze_date}. Last updated: {c.updated_at?.slice(0, 10)}.
      </p>
    </div>
  )
}
